...
class RestoController
{
    ...
    public function viewResto(Request $request, Response $response, array $parm)
    {
        ...
        if (($result->rowCount()) > 0
            ...
            $ordered_days = array_merge(array_slice($days_in_order, $current_day_index), array_slice($days_in_order, 0, $current_day_index));

            // Build the opening_hours array with status only for today
            $opening_hours = array_map(function($day) use ($week_schedule, $today, $current_time) {
                $open_time = $week_schedule[$day];
                $status = "tutup"; // Default to empty for non-today days

                // Set status only for today
                if ($day === $today && strpos($open_time, '–') !== false) {
                    list($start, $end) = explode('–', $open_time);
                    $status = ($current_time >= $start && $current_time <= $end) ? "buka" : "tutup";
                }
            ...
            $attributes = [
                ...
                "accessibility" => $data_row["aksesibilitas"],
